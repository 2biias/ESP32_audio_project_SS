...
void Bluetooth::bt_a2d_sink_cb(esp_a2d_cb_event_t event, esp_a2d_cb_param_t *param)
{
  ...
  // When a new device connects, set i2s sample rate accordingly
  if (a2d_event == ESP_A2D_CONNECTION_STATE_EVT) {
    uint32_t samplerate;
    if (sbc0 & 0x80) samplerate=16000;
    if (sbc0 & 0x40) samplerate=32000;
    if (sbc0 & 0x20) samplerate=44100;
    if (sbc0 & 0x10) samplerate=48000;
