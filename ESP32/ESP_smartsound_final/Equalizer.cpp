...
Equalizer::Equalizer(Ringbuffer<uint8_t>* read, Ringbuffer<short>* write)
{
  ...
  Lowshelf.fc = 88;
  Lowshelf.Q = 0.5;
  Lowshelf.A = 1.059; // 10 dB  - 10^(dB/40)
  ...
  for (size_t i = 0; i < 6; i++) {
    ...
    Bells.Q[i] = 0.5;
    Bells.A[i] = 1.059; //*correction; // 10 dB
  ...
  xTaskCreate(this->taskWrapper, "EqualizerTask", 30000, this, configMAX_PRIORITIES-1, &taskHandle);
...
void Equalizer::EqualizerTask()
{
  ...
  uint8_t inputbuffer[MAXSAMPLES] = {0}; // For receiving raw bytes
  short outputbuffer[MAXSAMPLES/4] = {0}; // For mono-mixed 16-bit samples
  float accumulator[MAXSAMPLES/4] = {0}; // For accumulating equalized samples
  ...
  for(;;){
    ...
    bytes = read(inputbuffer, MAXSAMPLES);
    ...
    for (size_t i = 0; i < len; i++){
      outputbuffer[i] = (short)(accumulator[i]*0.027); // (-13dB)
    }
...
void Equalizer::taskWrapper(void* _this){
    static_cast<Equalizer*>(_this)->EqualizerTask();
}
