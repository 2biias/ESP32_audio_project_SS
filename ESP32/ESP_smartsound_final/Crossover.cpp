...
{
  uint32_t len = 0;
  short inputbuffer[MAXSAMPLES/4] = {0};
  short outputbuffer[MAXSAMPLES/2] = {0};
  ...
  for(;;){
    ...
    len = read(inputbuffer, MAXSAMPLES/4);
...
uint32_t Crossover::filter_LinkwitzRiley(short* input, short* output, uint32_t len)
{
  ...
  for (size_t i = 0; i < len; i++) {
    ...
    output[2*i] = (short)(accumulator*0.98);
    ...
    output[(2*i)+1] = (short)(accumulator*0.98);
  }
  return len;
}
